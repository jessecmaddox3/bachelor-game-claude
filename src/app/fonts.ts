...
import montserratUrl from '../assets/fonts/Montserrat-Regular.ttf?url';
...
export async function loadAppFonts(): Promise<AppFonts> {
  const [display, body, bodyBold, landscape, landscapeBold] = await Promise.all(
    [archivoUrl, latoUrl, latoBoldUrl, montserratUrl, montserratBoldUrl].map(async (u) => (await fetch(u)).arrayBuffer()),
  );
  const buffers: FontBuffers = { display: display!, body: body!, bodyBold: bodyBold!, landscape: landscape!, landscapeBold: landscapeBold! };
  const faces = [
    ...
    new FontFace('Montserrat', buffers.landscape, { weight: '400' }),
    new FontFace('Montserrat', buffers.landscapeBold, { weight: '700' }),
  ];
